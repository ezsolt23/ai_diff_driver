...
from sac_criticQ import CriticQ
from sac_criticV import CriticV
...
class Agent:
    ...
    def __init__(self, input_dim, output_dim, lr, gamma, tau, alpha, clipnorm, verbose):
        ...
        clipnorm_val = self.clipnorm_val
        ...
        self.actor = Actor(input_dim, output_dim, lr, gamma,
                           tau, alpha, clipnorm, clipnorm_val, verbose)
        self.criticQ = CriticQ(input_dim, output_dim, lr,
                               gamma, tau, alpha, clipnorm, clipnorm_val, verbose)
        self.criticV = CriticV(input_dim, output_dim, lr,
                               gamma, tau, alpha, clipnorm, clipnorm_val, verbose)
    ...
    def learn(self):
        ...
        # reshape to np.array([[x1],[x2],[x3]]), same as Q, V_target.
        Pi = np.array([[i] for i in Pi])
        ...
        Pi_vec = self.actor.model.predict(S)
        # now a batch of \pi(s_t,a_t)
        Pi = np.sum(Pi_vec*A_current_policy, axis=1)
        # reshape to np.array([[x1],[x2],[x3]]), same as Q, V_target.
        Pi = np.array([[i] for i in Pi])
    ...
    def save_target_weights(self):
        ...
        # save attached the extension
        pars_tag = '_gamma_' + str(self.gamma) + '_lr_' + \
            str(self.lr) + '_tau_' + str(self.tau)
    ...
    def load_target_weights(self, gamma, lr, tau):
        ...
        # Now save the weights. I'm choosing ID by gamma, lr, tau
        pars_tag = '_gamma_' + str(gamma) + '_lr_' + \
            str(lr) + '_tau_' + str(tau) + '.npy'
